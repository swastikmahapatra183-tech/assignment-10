...
def election_winner(votes):
    ...
    winners = [candidate for candidate, count in vote_count.items()
               if count == max_votes]
    if len(winners) == 1:
        print(f"Winner: {winners[0]} ({max_votes} votes)")
    else:
        winners.sort()
        print(f"Tie between: {winners} ({max_votes} votes each)")

input-1
votes1 = ['a', 'b', 'a', 'c', 'a']
election_winner(votes1)

print()

input-2
votes2 = ['a', 'b', 'a', 'b', 'c']
election_winner(votes2)
